...
class WebGraph():
    ...
    def search(self, pi, query='', max_results=10, sum_weight=0.03, power=30):
        '''
        ...
        for i in range(n):
            ...
            urls = self._index_to_url(i)

            if new_url_satisfies_query(urls, query):
                number += 1
                weight += sum_weight
            ...
            for word in range(10):
                ...
                if new_url_satisfies_query(urls, W):
                    number += 1
                    weight += S[word][1]**power
...
def new_url_satisfies_query(url,query):
    satisfies = False
    terms = query.split()
...
def url_satisfies_query(url, query):
    ...
    for word in query.split():
        if '-' in word:
            ...
            similar_words = vectors.most_similar(word,topn=5)
            for i in range(len(similar_words)):
                terms.append(similar_words[i][0])
...
if __name__=='__main__':
    ...
    parser.add_argument('--sum_weight', type=float, default=0.03)
    ...
    g.search(pi, query=args.search_query, max_results=args.max_results, sum_weight=args.sum_weight, power=args.power)
